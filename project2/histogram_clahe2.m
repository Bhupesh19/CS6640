function [res] = histogram_clahe2(varargin)
...
% Usage: H = histogram_clahe1(I, numBins, windowSize, clipLevel)
  ...
  for i = 0:numTileX
    for j = 0:numTileY
      ...
      for y=winSize(1)/2:winSize(1)+winSize(1)/2
          for x=winSize(2)/2:winSize(2)+winSize(2)/2
              pixVal = floor(I2(i*winSize(1)+y,j*winSize(2)+x)/stepSize)+1;
              fU = histUL(pixVal)*(1.5*winSize(1)-x) + ...
                    histUR(pixVal)*(x-0.5*winSize(1));
              fD = histDL(pixVal)*(1.5*winSize(1)-x) + ...
                    histDR(pixVal)*(x-0.5*winSize(1));
                
              I2(i*winSize(1)+y, j*winSize(2)+x) = (fU*(1.5*winSize(1)-y) + ...
                        fD*(y-0.5*winSize(1))) * (255 / winPixels);
          end
      end
